...
export default function Navbar() {
    ...
    return (
        <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.6 }} className={`fixed w-full z-50 rounded-b-sm shadow-lg border-b-2 border-purple800 bg-purple800 ${scrolled ? 'shadow-md py-4' : 'py-6'}`}>
            <div className="max-w-8xl mx-auto px-6 flex items-center justify-between text-purple100">
                ...
                <Link to="/" className="text-2xl font-bold tracking-wider flex gap-2 items-center absolute left-1/2 -translate-x-1/2">
                ...
                <div className="flex items-center gap-4">
                    {user ? (
                        <>
                            ...
                            <button onClick={() => { logout(); nav('/'); }} className="text-white"><LogOut className="w-5 h-5" /></button>
                        </>
                    ) : (
                        <Link to="/login" className="text-white flex items-center gap-1 text-sm"><User className="w-5 h-5" /> Login</Link>
                    )}
                    <Link to="/cart" className="text-white relative"><ShoppingBag className="w-6 h-6" /><span className="absolute -top-2 -right-2 bg-purple200 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{count}</span></Link>
